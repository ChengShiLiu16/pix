...
const agentDirEnvName = "PIX_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "PIX_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific PIX_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force PIX_OFFLINE=1 / PIX_SKIP_VERSION_CHECK=1
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "pix-startup-benchmark-")) : undefined;
