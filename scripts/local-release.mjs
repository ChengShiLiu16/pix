...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `pix-${platform}.zip` : `pix-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createPixShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "pix.cmd"))) {
			writeFileSync(join(installDirectory, "pix.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\pix.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "pix.ps1"), '& "$PSScriptRoot/node_modules/.bin/pix.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "pix.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\pix.exe" %*\r\n');
		writeFileSync(join(installDirectory, "pix.ps1"), '& "$PSScriptRoot/node_modules/.bin/pix.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "pix"), join(installDirectory, "pix"));
}
...
if (!options.skipInstall) {
	...
	createPixShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createPixShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `pix-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "pix.exe" : "pix")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "pix.cmd" : "pix")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "pix.cmd" : "pix")} --help`);
	}
}
