 ...
 *   --analyze      Spawn pix subagents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("pix", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn pix: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".pix/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn pix subagents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".pix/agent/AGENTS.md");
