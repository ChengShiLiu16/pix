#!/usr/bin/env bash
#
# Build pix binaries for all platforms locally.
...
#     pix-darwin-arm64.tar.gz
#     pix-darwin-x64.tar.gz
#     pix-linux-x64.tar.gz
#     pix-linux-arm64.tar.gz
#     pix-windows-x64.zip
#     pix-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/pix.exe"
    else
        bun build --compile --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/pix"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating pix-$platform.zip..."
        (cd "$platform" && zip -r ../pix-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating pix-$platform.tar.gz..."
        mv "$platform" pix && tar -czf pix-$platform.tar.gz pix && mv pix "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../pix-$platform.zip)
    else
        tar -xzf pix-$platform.tar.gz && mv pix "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/pix.exe"
    else
        echo "  $OUTPUT_DIR/$platform/pix"
    fi
done
