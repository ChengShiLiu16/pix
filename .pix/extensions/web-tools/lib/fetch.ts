...
function isHtmlContent(contentType: string | undefined): boolean {
	return !contentType || contentType.toLowerCase().includes("text/html") || contentType.toLowerCase().includes("application/xhtml+xml");
}
...
export async function webFetch(input: WebFetchInput, options: WebFetchOptions = {}): Promise<WebFetchResult> {
	...
	try {
		const { response, finalUrl } = await fetchWithRedirects(initialUrl, options.fetchFn ?? defaultFetch(), timeout.signal);
		if (!response.ok) {
		...
		if (!isHtmlContent(contentType)) {
			...
			return { url: initialUrl.toString(), finalUrl, contentType, content: plain.content, truncated: plain.truncated || bodyTruncated };
		}
