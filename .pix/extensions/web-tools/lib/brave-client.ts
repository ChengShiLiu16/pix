...
export async function fetchBraveSearchResults(request: BraveSearchRequest): Promise<WebSearchResultItem[]> {
	...
	try {
		const response = await (request.fetchFn ?? defaultFetch())(`https://api.search.brave.com/res/v1/web/search?${params}`, {
			headers: {
		...
		if (!response.ok) {
			...
			throw new WebToolsError(`Brave Search request failed: HTTP ${response.status} ${response.statusText}${errorText ? `\n${errorText}` : ""}`);
		}
