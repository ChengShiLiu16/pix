...
export function getWebToolsConfig(env: WebToolsEnv = process.env): WebToolsConfig {
	...
	const maxFetchMaxChars = readInteger(env, "PI_WEB_FETCH_MAX_CHARS", DEFAULT_WEB_TOOLS_CONFIG.maxFetchMaxChars, 1000, 100000);
	...
	return {
		...
		defaultSearchCount: readInteger(env, "PI_WEB_SEARCH_DEFAULT_COUNT", DEFAULT_WEB_TOOLS_CONFIG.defaultSearchCount, 1, maxSearchCount),
		maxSearchCount,
		searchTimeoutMs: readInteger(env, "PI_WEB_SEARCH_TIMEOUT_MS", DEFAULT_WEB_TOOLS_CONFIG.searchTimeoutMs, 1000, 60000),
		fetchTimeoutMs: readInteger(env, "PI_WEB_FETCH_TIMEOUT_MS", DEFAULT_WEB_TOOLS_CONFIG.fetchTimeoutMs, 1000, 60000),
		defaultFetchMaxChars: readInteger(env, "PI_WEB_FETCH_DEFAULT_MAX_CHARS", DEFAULT_WEB_TOOLS_CONFIG.defaultFetchMaxChars, 1000, maxFetchMaxChars),
		maxFetchMaxChars,
	};
}
