...
async function htmlToMarkdown(html: string): Promise<string> {
	try {
		...
		throw new WebToolsError(`/web markdown extraction dependencies are missing or failed to load: ${error instanceof Error ? error.message : String(error)}`);
	}
}
...
async function extractFallbackContent(
	...
	const content = format === "text"
		...
		: await htmlToMarkdown(root?.innerHTML ?? "");
...
export async function extractReadableContent(
	...
	try {
		...
		throw new WebToolsError(`/web content extraction failed: ${error instanceof Error ? error.message : String(error)}`);
	}
}
