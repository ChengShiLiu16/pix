...
export default function (pi: ExtensionAPI): void {
	pi.registerTool<typeof WebSearchParams>({
		...
		promptGuidelines: [
			"需要联网信息时使用 Brave_Search",
			"用户提供 URL 则 action='fetch'，否则 action='search'",
		],
		...
		async execute(_toolCallId, params, signal, _onUpdate, _ctx) {
			if (params.action === "fetch") {
				...
				try {
					...
					return { content: [{ type: "text", text: formatFetchResult(result) }] };
				} catch (error) {
					return { content: [{ type: "text", text: `抓取失败：${errorMessage(error)}` }], isError: true };
				}
			}
			...
			try {
				...
				return { content: [{ type: "text", text: `搜索失败：${errorMessage(error)}` }], isError: true };
			}
		},
	});
}
