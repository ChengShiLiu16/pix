...
export type RawTaskParams = {
  ...
  agent?: string;
  background?: boolean;
  tasks?: Array<{ prompt?: string; task?: string; instruction?: string; instructions?: string; description?: string; agent?: string }>;
};

/** Resolve worker prompt from common aliases (prompt, task, instruction, …). */
export function resolveTaskPrompt(params: Pick<RawTaskParams, "prompt" | "task" | "instruction" | "instructions">): string | undefined {
