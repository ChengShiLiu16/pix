...
const TaskItem = Type.Object({
  prompt: Type.Optional(Type.String({ description: "REQUIRED unless task/instruction/instructions is provided. Full instructions for this worker." })),
...
const TaskParams = Type.Object({
  prompt: Type.Optional(Type.String({
    description: "REQUIRED for single-task mode. Full worker instructions. Every parallel task() call must include its own prompt — description alone is not enough.",
  })),
...
async function writePromptFile(name: string, text: string): Promise<string | undefined> {
  ...
  await fs.promises.writeFile(file, text, { encoding: "utf-8", mode: 0o600 });
  return file;
}
...
function startSubprocess(cwd: string, spawnArgs: string[], signal?: AbortSignal) {
  ...
  if (signal) {
    const kill = () => { aborted = true; proc.kill("SIGTERM"); setTimeout(() => { if (!proc.killed) proc.kill("SIGKILL"); }, 3000); };
...
async function runWorkerWithRetry(
  ...
  args: string[],
  signal?: AbortSignal,
  onProc?: (proc: ChildProcessWithoutNullStreams) => void,
  ...
  if (
    !firstCheck.ok &&
    result.metrics.durationMs < FAST_FAIL_MS &&
    !signal?.aborted
  ) {
...
function hydrateWorkerRecord(rec: WorkerRecord): WorkerRecord {
...
export default function (pi: ExtensionAPI) {
  ...
  const updateWidget = () => {
    ...
    for (const w of list) {
      ...
      if (w.status !== "running" && w.durationMs != null) {
        const timing = w.status === "failed" ? `failed in ${formatDuration(w.durationMs)}` : formatDuration(w.durationMs);
        line += ` · ${timing}`;
  ...
  async function runTask(
    ...
    if (background) {
      ...
      }).finally(async () => { backgroundRunning = Math.max(0, backgroundRunning - 1); await cleanupPromptFile(promptFile); });
  ...
  pi.on("session_shutdown", async (_e, ctx) => {
    workers.forEach((w) => w.proc?.kill("SIGTERM"));
    workers.clear();
    terminalInputUnsub?.();
    terminalInputUnsub = undefined;
  ...
  pi.registerTool({
    ...
    renderResult(result, _opts, theme, context) {
      ...
      const body = stored.content?.[0]?.type === "text" ? stored.content[0].text : result.content[0]?.type === "text" ? result.content[0].text : "";
      ...
      const details = resolveTaskDetails(
        storedDetails,
        live ? buildTaskDetails(live) : undefined,
        completed,
      );
    ...
    async execute(_id, params: RawTaskParams, signal, onUpdate, ctx) {
      if (!multitaskEnabled) {
        return { content: [{ type: "text", text: buildTaskDisabledReason() }], isError: true };
      }
      widgetCtx = ctx;
      agents = loadAgents();
      ...
      if (params.tasks?.length) {
        ...
        if (missing >= 0) {
          return {
            content: [{ type: "text", text: `Error: tasks[${missing}].prompt is required (full worker instructions).` }],
            isError: true,
          };
        }
      ...
      if (!prompt) {
        return { content: [{ type: "text", text: buildMissingPromptError(params) }], isError: true };
      }
      if (params.background && backgroundRunning >= MAX_BACKGROUND) {
        return { content: [{ type: "text", text: `Error: max ${MAX_BACKGROUND} background workers` }], isError: true };
      }
      try {
        ...
        return {
          ...
          details: rec ? buildTaskDetails(rec) : {
            workerId: r.workerId,
            status: r.status,
            output: r.output,
            stderr: r.stderr,
