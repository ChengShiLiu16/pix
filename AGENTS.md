...
- Never execute `rm -rf`, `sudo`, `git reset --hard`, `git checkout .`, `git clean -fd`, `git stash`
- Never modify .env, .git/, credentials or other sensitive files
- Run `git status` before committing
- Only `git add` files you modified, never use `git add -A` / `git add .`
...
- When uncertain, ask the user instead of assuming
...
Multiple pix sessions may be running in this cwd at the same time, each modifying different files. Git operations that touch unstaged, staged, or untracked files outside your own changes will stomp on other sessions' work.
...
- Pre-commit blocks lockfile commits unless `PIX_ALLOW_LOCKFILE_CHANGE=1`. Don't bypass unless the user wants the lockfile change committed
...
- Internal (from issues): `Fixed foo bar ([#123](https://github.com/ChengShiLiu16/pix/issues/123))`
- External contributions: `Added feature X ([#456](https://github.com/ChengShiLiu16/pix/pull/456) by [@username](https://github.com/username))`
...
3. **Run the release script**:
 ```bash
	PIX_ALLOW_LOCKFILE_CHANGE=1 npm_config_min_release_age=0 npm run release:patch    # fixes + additions
	PIX_ALLOW_LOCKFILE_CHANGE=1 npm_config_min_release_age=0 npm run release:minor    # breaking changes
 ```
...
## Testing pix Interactive Mode with tmux
