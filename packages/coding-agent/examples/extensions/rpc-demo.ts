...
export default function (pix: ExtensionAPI) {
	...
	pix.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "pix RPC Demo (new session)" : "pix RPC Demo");
	...
	pix.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	pix.on("turn_end", async (_event, ctx) => {
	...
	pix.registerCommand("rpc-editor", {
	...
	pix.registerCommand("rpc-prefill", {
