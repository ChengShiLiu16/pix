...
async function resolveGitHubRepo(pix: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await pix.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (pix: ExtensionAPI): void {
	pix.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(pix, ctx.cwd);
		if (!resolvedRepo.ok) {
