 ...
 *
 * Usage:
 *   pix -e ./ssh.ts --ssh user@host
 *   pix -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (pix: ExtensionAPI) {
	pix.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	pix.on("session_start", async (_event, ctx) => {
		...
		const arg = pix.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
