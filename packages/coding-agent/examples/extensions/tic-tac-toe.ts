...
export default function (pix: ExtensionAPI) {
	pix.on("session_start", async (_event, ctx) => reconstructState(ctx));
	pix.on("session_tree", async (_event, ctx) => reconstructState(ctx));
	...
	// -----------------------------------------------------------------------
	pix.registerMessageRenderer(MOVE_MESSAGE_TYPE, (message, { expanded }, theme) => {
	...
	// -----------------------------------------------------------------------
	pix.registerMessageRenderer(GAME_OVER_MESSAGE_TYPE, (message, _options, theme) => {
		const details = message.details as BoardDetails | undefined;
...
`;
	...
	// /tic-tac-toe command
	// -----------------------------------------------------------------------
	pix.registerCommand("tic-tac-toe", {
		...
		handler: async (_args, ctx) => {
			...
			gameActive = true;
			pix.setSessionName("Tic-Tac-Toe");
			...
			await ctx.ui.custom<void>((tui, _theme, _kb, done) => {
				component = new TicTacToeComponent(
					...
					(row, col) => {
						...
						pix.appendEntry(SAVE_TYPE, getBoardDetails());
