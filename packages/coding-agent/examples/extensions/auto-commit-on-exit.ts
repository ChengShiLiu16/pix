...
export default function (pix: ExtensionAPI) {
	pix.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await pix.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[pix] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await pix.exec("git", ["add", "-A"]);
		const { code: commitCode } = await pix.exec("git", ["commit", "-m", commitMessage]);
