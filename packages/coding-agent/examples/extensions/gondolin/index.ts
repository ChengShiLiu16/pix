 ...
 *
 * Runs pix's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   pix -e /path/to/pix/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (pix: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `pix ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	pix.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
