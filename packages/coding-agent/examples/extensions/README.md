...
Example extensions for pix-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
pix --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.pix/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `pix.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs pix theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `pix.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using pix-ai's built-in Anthropic/OpenAI streaming via proxy |
