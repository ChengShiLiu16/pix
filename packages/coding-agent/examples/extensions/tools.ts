 ...
 *
 * Usage:
 * 1. Copy this file to ~/.pix/agent/extensions/ or your project's .pix/extensions/
...
export default function toolsExtension(pix: ExtensionAPI) {
	// Track enabled tools
	...
	function persistState() {
		pix.appendEntry<ToolsState>("tools-config", {
	...
	function applyTools() {
		pix.setActiveTools(Array.from(enabledTools));
	}

	// Find the last tools-config entry in the current branch
	function restoreFromBranch(ctx: ExtensionContext) {
		allTools = pix.getAllTools();
		...
		if (savedTools) {
			...
			enabledTools = new Set(pix.getActiveTools());
		}
	}

	// Register /tools command
	pix.registerCommand("tools", {
