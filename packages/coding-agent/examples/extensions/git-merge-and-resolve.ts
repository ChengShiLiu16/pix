 ...
 *
 * Start pix with this extension:
 *   pix -e ./examples/extensions/git-merge-and-resolve.ts
 */
...
async function findConflicts(pix: ExtensionAPI, cwd: string): Promise<ConflictBlock[]> {
	const { stdout, code } = await pix.exec("git", ["diff", "--name-only", "--diff-filter=U"]);
...
export default function (pix: ExtensionAPI) {
	pix.on("agent_end", async (_event, ctx) => {
		const { code: revParseCode } = await pix.exec("git", ["rev-parse", "--git-dir"]);
		if (revParseCode !== 0) return;

		let ref = "MERGE_HEAD";

		// If not already in a merge, attempt one
		const { code: mergeHeadCode } = await pix.exec("git", ["rev-parse", "MERGE_HEAD"]);
		if (mergeHeadCode !== 0) {
			// Only attempt a new merge if the working tree is clean
			const { stdout: status } = await pix.exec("git", ["status", "--porcelain"]);
			if (status.trim()) return;

			const { stdout: upstream, code: upstreamCode } = await pix.exec("git", [
				"rev-parse",
				"--abbrev-ref",
				"--symbolic-full-name",
				"@{u}",
			...
			const { code: fetchCode, stderr: fetchErr } = await pix.exec("git", ["fetch", remote]);
			if (fetchCode !== 0) {
				ctx.ui.notify(`git-merge-and-resolve: fetch failed: ${fetchErr.trim()}`, "warning");
				return;
			}

			const { code: mergeCode } = await pix.exec("git", ["merge", "--no-ff", ref]);
			if (mergeCode === 0) return;
		}

		// Either we just merged with conflicts, or we were already in an unfinished merge
		const conflicts = await findConflicts(pix, ctx.cwd);
		if (conflicts.length === 0) return;

		pix.sendUserMessage(formatConflicts(ref, conflicts), { deliverAs: "followUp" });
	});
}
