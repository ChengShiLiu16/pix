 ...
 *
 * Usage: pix --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@chengshiliu16/pix-coding-agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@chengshiliu16/pix-tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@chengshiliu16/pix-tui";
...
export default function (pix: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like a game overlay would need)
	pix.registerCommand("overlay-animation", {
	...
	pix.registerCommand("overlay-anchors", {
	...
	pix.registerCommand("overlay-margins", {
	...
	pix.registerCommand("overlay-stack", {
	...
	pix.registerCommand("overlay-overflow", {
	...
	pix.registerCommand("overlay-edge", {
	...
	pix.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	pix.registerCommand("overlay-maxheight", {
	...
	pix.registerCommand("overlay-sidepanel", {
	...
	pix.registerCommand("overlay-toggle", {
	...
	pix.registerCommand("overlay-passive", {
	...
	pix.registerCommand("overlay-focus", {
	...
	pix.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/pix/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/pix\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like a game overlay
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (game overlays use the same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
