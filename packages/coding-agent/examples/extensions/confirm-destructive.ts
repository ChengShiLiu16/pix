...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@chengshiliu16/pix-coding-agent";

export default function (pix: ExtensionAPI) {
	pix.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
