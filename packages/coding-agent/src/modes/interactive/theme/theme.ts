...
function getBuiltinThemes(): Record<string, ThemeJson> {
	if (!BUILTIN_THEMES) {
		...
		const noBgDarkPath = path.join(themesDir, "no-bg-dark.json");
		BUILTIN_THEMES = {
			dark: JSON.parse(fs.readFileSync(darkPath, "utf-8")) as ThemeJson,
			light: JSON.parse(fs.readFileSync(lightPath, "utf-8")) as ThemeJson,
			"no-bg-dark": JSON.parse(fs.readFileSync(noBgDarkPath, "utf-8")) as ThemeJson,
		};
	}
...
const THEME_KEY = Symbol.for("@chengshiliu16/pix-coding-agent:theme");
...
export function getResolvedThemeColors(themeName?: string): Record<string, string> {
	const requested = themeName ?? currentThemeName ?? getDefaultTheme();
	let name = requested;
	let themeJson: ThemeJson;
	try {
		themeJson = loadThemeJson(name);
	} catch {
		name = currentThemeName ?? getDefaultTheme();
		themeJson = loadThemeJson(name);
	}
