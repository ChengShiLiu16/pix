...
function _firstMeaningfulLine(text: string): string {
...
function summarizeThinkingTextDetailed(
	...
	if (
		...
		challenger.hasExplicitSuccess &&
		hasExplicitSuccessCue(challengerSummary) &&
		(baselineHasExplicitFailure || !baselineHasExplicitSuccess)
	) {
...
export function inferThinkingRole(text: string): ThinkingSemanticRole {
	...
	const scoredRoles: Array<{ role: ThinkingSemanticRole; score: number }> = [
		...
		},
		{
			...
			score:
				Number(
					/\b(compare|comparison|versus|\bvs\b|trade-?off|alternative|option|weigh|choose between)\b/.test(
						haystack,
					),
				) * 4,
		},
		...
		},
		{
			...
			score:
				Number(
					/\b(plan|planning|approach|strategy|outline|decide|figure out|map out|organize|break down)\b/.test(
						haystack,
					),
				) * 3,
		},
