import { truncateToWidth, visibleWidth, wrapTextWithAnsi } from "@chengshiliu16/pix-tui";
...
export const THINKING_PARSE_CHAR_LIMIT = 8000;

interface OversizedThinkingInfo {
	totalChars: number;
	blockCount: number;
	limit: number;
}
...
export function getOversizedThinkingInfo(
	blocks: ThinkingSourceBlock[],
	limit = THINKING_PARSE_CHAR_LIMIT,
): OversizedThinkingInfo | undefined {
	let totalChars = 0;
	for (const block of blocks) {
		totalChars += block.text.length;
	}
	return totalChars > limit ? { totalChars, blockCount: blocks.length, limit } : undefined;
}

function renderOversizedThinkingNotice(
	theme: ThinkingThemeLike,
	width: number,
	info: OversizedThinkingInfo,
	isActive: boolean,
	nowMs = Date.now(),
): string[] {
	...
	const message = [
		"Thinking parse skipped",
		`${formatNumber(info.totalChars)} chars`,
		`${info.blockCount} ${blockLabel}`,
		`limit ${formatNumber(info.limit)}`,
	].join(" · ");
	const line = `${theme.fg("muted", "│")} ${theme.fg("dim", message)} ${activity}`;
	return [truncateToWidth(line, width, "")];
}

function _stepHeader(theme: ThinkingThemeLike, step: DerivedThinkingStep, active: boolean, connector: string): string {
	...
	return `${theme.fg("muted", connector)} ${icon} ${summaryText}`;
}
...
function wrapStepHeader(
	...
	const icon = theme.fg(roleColor(step.role), step.icon);
	const prefix = `${theme.fg("muted", connector)} ${icon} `;
...
function _stripInlineFormattingMarkers(text: string): string {
	return text
...
function renderExpanded(
	...
	const lines = [truncateToWidth(`${theme.fg("muted", "┆")} ${theme.fg("dim", "Thinking Steps · Expanded")}`, width)];
...
export class ThinkingStepsComponent {
	private steps: DerivedThinkingStep[];
	private oversized?: OversizedThinkingInfo;
	...
	constructor(theme: ThinkingThemeLike, messageTimestamp: number, blocks: ThinkingSourceBlock[], scopeKey?: string) {
		this.theme = theme;
		this.messageTimestamp = messageTimestamp;
		this.oversized = getOversizedThinkingInfo(blocks);
		this.steps = this.oversized ? [] : deriveThinkingSteps(blocks);
	...
	render(width: number): string[] {
		...
		if (this.oversized) {
			return renderOversizedThinkingNotice(this.theme, width, this.oversized, active.active, Date.now());
		}
		const activeStepId =
