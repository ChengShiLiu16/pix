...
import {
	clearThinkingStepsModePreference,
	readThinkingStepsModePreference,
	writeThinkingStepsModePreference,
} from "./persistence.ts";
import {
...
import type { PersistedThinkingStepsPreferenceScope, ThinkingStepsMode } from "./types.ts";
...
const ROOT_COMPLETION_OPTIONS = [...MODE_OPTIONS, ...SCOPE_OPTIONS, "status"];
...
function invalidUsageMessage(): string {
	return "Usage: /thinking-steps [collapsed|summary|expanded|status] | [project|global] [collapsed|summary|expanded|clear]";
}
...
function thinkingModeCompletions(prefix: string): AutocompleteItem[] | null {
	...
	if (!trimmed) {
		return ROOT_COMPLETION_OPTIONS.map((value) => ({ value, label: value }));
	}

	const parts = trimmed.split(/\s+/);
	if (parts.length === 1 && !endsWithWhitespace) {
		return buildCompletionItems(ROOT_COMPLETION_OPTIONS, parts[0] ?? "");
	}
...
export function builtin(pi: ExtensionAPI): void {
	...
	const degradedSessionMessage = (): string =>
		"Thinking steps is using Pi's native thinking renderer for this session; live mode switching is disabled.";
	const futureCompatibleSessionMessage = (
		scope: PersistedThinkingStepsPreferenceScope,
		action: "saved" | "cleared",
	): string =>
		`${action === "saved" ? "Saved" : "Cleared"} ${scope} thinking view default for future compatible sessions; the current session is using Pi's native thinking renderer.`;
	...
	pi.registerCommand("thinking-steps", {
		...
		handler: async (args, ctx) => {
			...
			if (action.type === "status") {
				const patchStatus = degraded ? "degraded" : getPatchCleanup() ? "installed" : "not installed";
				notifyUser(
					ctx,
					[
						`Thinking view: ${getThinkingStepsMode(ctx.cwd)}`,
						`Patch: ${patchStatus}`,
						`Patch refs: ${getPatchRefCount()}`,
						`Scope: ${ctx.cwd}`,
					].join("\n"),
	...
	pi.on("message_start", async (event) => {
		if (event.message.role === "assistant") {
			...
			const timestamp =
				typeof (event.message as { timestamp?: unknown }).timestamp === "number"
	...
	pi.on("message_update", async (event) => {
		...
		if (assistantEvent.type === "thinking_delta") {
			const delta =
				"delta" in assistantEvent && typeof assistantEvent.delta === "string" ? assistantEvent.delta : "";
			if (delta.trim()) {
				setActiveThinkingState(
					{
						active: true,
