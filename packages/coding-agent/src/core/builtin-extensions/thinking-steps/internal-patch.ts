...
function _fallbackToOriginalUpdateContent(
...
function getPackageRoot(packageName: string): string {
	...
	try {
		...
		throw new Error(
			`Thinking Steps patch failed: could not resolve ${packageName} package root. Pi internals may be unavailable or moved.`,
			{
				cause: error,
			},
		);
...
export async function importPiCodingAgentInternal<TModule>(relativePath: string): Promise<TModule> {
	...
	try {
		...
		throw new Error(
			`Thinking Steps patch failed: could not import internal module "@earendil-works/pi-coding-agent/${relativePath}". Pi internals may have moved.`,
			{
				cause: error,
			},
		);
...
function hasVisibleTextContent(message: AssistantMessage): boolean {
	return message.content.some((content) => content.type === "text" && /\S/u.test(content.text));
}

function _hasVisibleThinkingContent(message: AssistantMessage): boolean {
