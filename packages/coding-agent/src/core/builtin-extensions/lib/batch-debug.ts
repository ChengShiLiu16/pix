 ...
 * Enable with PI_BATCH_DEBUG=1 or `/batch-debug on`.
 */
let runtimeDebugEnabled = process.env.PI_BATCH_DEBUG === "1";
...
export function isBatchDebugEnabled(): boolean {
	return runtimeDebugEnabled || process.env.PI_BATCH_DEBUG === "1";
}
