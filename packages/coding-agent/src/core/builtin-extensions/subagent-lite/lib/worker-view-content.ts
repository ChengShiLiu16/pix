import { isParallelTaskDetails, type ParallelTaskDetails, type TaskDetails } from "./task-render.ts";
...
function formatSubResultsSectionPlain(subResults: SubResult[] | undefined): string {
	...
	for (let i = 0; i < subResults.length; i++) lines.push(formatSubResultBlock(subResults[i]!, i + 1));
	return lines.join("\n");
}
