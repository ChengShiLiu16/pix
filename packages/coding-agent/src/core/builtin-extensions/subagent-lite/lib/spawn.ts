...
export function buildSpawnArgs(agent: AgentConfig | undefined, prompt: string, options: SpawnOptions = {}): string[] {
	const args = [
		"--mode",
		"json",
		"-p",
...
export function assessWorkerSuccess(exitCode: number, output: string, metrics: WorkerRunMetrics): WorkerAssessment {
	...
	if (metrics.durationMs < FAST_FAIL_MS && metrics.toolCalls === 0) {
		return {
			ok: false,
			reason: `no tool activity in ${formatDuration(metrics.durationMs)} — worker likely did not run`,
		};
	}
