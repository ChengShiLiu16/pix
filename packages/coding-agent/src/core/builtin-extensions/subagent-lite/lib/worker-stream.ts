...
export function createWorkerStreamParser() {
	...
	const finalizeTool = (toolCallId: string | undefined, toolName: string, result: unknown, isError = false) => {
	...
	const snapshot = (): WorkerStreamSnapshot => ({
		messages,
		toolCalls: Math.max(toolCalls, subResults.length),
		subResults,
	});
