...
export const WORKER_VIEW_KEYMAP = IS_DARWIN
	? ["Ctrl+1..9 — 按最近顺序查看 worker（Ctrl+1 = 最新）", "/view-worker <id> — 按 id 查看（>9 个 worker 时）"].join(
			"\n· ",
		)
	: [
...
export function resolveWorkerShortcutIndex(idsMostRecentFirst: string[], workerId: string): number | undefined {
	const idx = idsMostRecentFirst.indexOf(workerId);
	if (idx < 0 || idx >= MAX_WORKER_SHORTCUTS) return undefined;
	return idx + 1;
}

export function selectWorkerIdByShortcutIndex(idsMostRecentFirst: string[], index: number): string | undefined {
