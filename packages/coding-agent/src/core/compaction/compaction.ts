...
import type { AgentMessage, StreamFn, ThinkingLevel } from "@chengshiliu16/pix-agent-core";
import { contentText, type RetryCallbacks, type RetryPolicy, retryAssistantCall, uuidv7 } from "@chengshiliu16/pix-ai";
import type { AssistantMessage, Context, Model, SimpleStreamOptions, Usage } from "@chengshiliu16/pix-ai/compat";
import { completeSimple } from "@chengshiliu16/pix-ai/compat";
...
function extractFileOperations(
	...
	// Collect from previous compaction's details (if pix-generated)
...
export async function generateSummaryWithUsage(
	...
	} else if (previousSummaryTokens > reserveTokens * 0.5) {
	...
	let textContent = contentText(response.content);
	...
	if (textContent && estimateTextTokens(textContent) > reserveTokens * 0.8) {
		...
		if (collapseResponse.stopReason !== "error") {
			const collapsed = contentText(collapseResponse.content);
			if (collapsed.trim()) {
				textContent = collapsed;
				usage = combineUsage(usage, collapseResponse.usage);
			}
		}
	}
...
export async function compact(
	...
	const resolvedSessionId = sessionId ?? env?.PIX_SESSION_ID;
	const {
		firstKeptEntryId,
		messagesToSummarize,
		turnPrefixMessages,
	...
	const compactTemplateUsed = Boolean(
		previousSummary && estimateTextTokens(previousSummary) > settings.reserveTokens * 0.5,
	);
	const doubleCompactTriggered = summaryTokensAfter > settings.reserveTokens * 0.8;
	if (resolvedSessionId) {
		emitCompaction({
			...
			droppedMessages: messagesToSummarize.length + turnPrefixMessages.length,
			compactTemplateUsed,
