...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@chengshiliu16/pix-coding-agent` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@chengshiliu16/pix-coding-agent";
	 *
...
export interface ExtensionContext {
	...
	/** Gracefully shutdown pix and exit. Available in all contexts. */
	shutdown(): void;
...
export interface BeforeAgentStartEvent {
	...
	systemPrompt: string;
	/** Structured options used to build the system prompt. Extensions can inspect this to understand what Pix loaded without re-discovering resources. */
...
/** Configuration for registering a provider via pix.registerProvider(). */
...
export type ExtensionFactory = (pix: ExtensionAPI) => void | Promise<void>;
...
/**
 * Action implementations for pix.* API methods.
