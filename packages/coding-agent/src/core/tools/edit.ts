...
export function createEditToolDefinition(
	...
	return {
		...
		label: "edit",
		description:
			"Edit a single file using exact text replacement. Each edits[].oldText must match a unique region of the original file; merge nearby or overlapping changes into one edit.",
		promptSnippet:
			"Make precise file edits with exact text replacement, including multiple disjoint edits in one call",
		promptGuidelines: [
			"When changing multiple separate locations in one file, use one edit call with multiple edits[] entries instead of multiple edit calls.",
			"Each edits[].oldText matches against the original file (not after earlier edits); keep it as small as possible while still unique, and do not overlap or nest edits.",
		],
