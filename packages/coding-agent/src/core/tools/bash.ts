...
import { Container, Text, truncateToWidth } from "@chengshiliu16/pix-tui";
...
/**
 * Create bash operations using pix's built-in local shell execution backend.
 *
 * This is useful for extensions that intercept user_bash and still want pix's
				...
				render: (width: number) => {
					...
					if (state.cachedSkipped && state.cachedSkipped > 0) {
						const hint =
							...
							` click or ${keyHint("app.tools.expand", "to expand")})`;
