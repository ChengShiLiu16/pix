...
import { contentText } from "@chengshiliu16/pix-ai";
import type {
	AssistantMessage,
	AuthResult,
	ImageContent,
	...
	TextContent,
	Usage,
} from "@chengshiliu16/pix-ai/compat";
import {
	clampThinkingLevel,
...
export class AgentSession {
	 ...
	 * - Handles extension commands (registered via pix.registerCommand) immediately, even during streaming
	...
	async prompt(text: string, options?: PromptOptions): Promise<void> {
		...
		try {
			...
			// Extension commands manage their own LLM interaction via pix.sendMessage()
		...
		try {
			...
			if (this._extensionRunner.hasHandlers("session_before_compact")) {
				...
				if (extensionResult?.customInstructions !== undefined) {
					effectiveCustomInstructions = extensionResult.customInstructions;
				}
			}
