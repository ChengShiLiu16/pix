...
import { type ImageContent, type Message, type TextContent, type Usage, uuidv7 } from "@chengshiliu16/pix-ai";
 ...
 * Encodes cwd into a safe directory name under ~/.pix/agent/sessions/.
 */
...
export class SessionManager {
		...
		if (existsSync(this.sessionFile)) {
			...
			if (this.fileEntries.length === 0) {
				...
				if (statSync(explicitPath).size > 0) {
					throw new Error(`Session file is not a valid Pix session: ${explicitPath}`);
				}
				this.newSession();
	 ...
	 * @param sessionDir Optional session directory. If omitted, uses default (~/.pix/agent/sessions/<encoded-cwd>/).
	 */
