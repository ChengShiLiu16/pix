...
By default, `install` and `remove` write to user settings (`~/.pix/agent/settings.json`). Use `-l` to write to project settings (`.pix/settings.json`) instead. Project settings can be shared with your team, and pix installs any missing packages automatically on startup after the project is trusted.
...
Add a `pix` manifest to `package.json` or use conventional directories. Include the `pix-package` keyword for discoverability.
...
The [package gallery](https://github.com/ChengShiLiu16/pix) displays packages tagged with `pix-package`. Add `video` or `image` fields to show a preview:
...
Pix bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@chengshiliu16/pix-ai`, `@chengshiliu16/pix-agent-core`, `@chengshiliu16/pix-coding-agent`, `@chengshiliu16/pix-tui`, `typebox`.
