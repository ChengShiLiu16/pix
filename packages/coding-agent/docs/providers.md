...
Pix supports subscription-based providers via OAuth and API key providers via environment variables or auth file. Built-in catalogs ship with Pix; configured providers may refresh newer catalogs and cache them in `~/.pix/agent/models-store.json` for offline use.
...
Use `/logout` to clear credentials. Tokens are stored in `~/.pix/agent/auth.json` and auto-refresh when expired. OpenRouter instead mints a user-controlled API key that does not expire automatically.
...
Reference for environment variables and `auth.json` keys: [`const envMap`](https://github.com/ChengShiLiu16/pix/blob/main/packages/ai/src/env-api-keys.ts) in [`packages/ai/src/env-api-keys.ts`](https://github.com/ChengShiLiu16/pix/blob/main/packages/ai/src/env-api-keys.ts).

#### Auth File

Store credentials in `~/.pix/agent/auth.json`:
...
```bash
pix --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
pix --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
...
pix --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
For normal pix usage, prefer unified billing or stored BYOK. Inline BYOK requires configuring an additional upstream `Authorization` header for the Cloudflare AI Gateway provider, for example via a `models.json` provider/model override.
...
export CLOUDFLARE_API_KEY=...           # or use /login
export CLOUDFLARE_ACCOUNT_ID=...
pix --provider cloudflare-workers-ai --model "@cf/moonshotai/kimi-k2.6"
```

Pix automatically sets `x-session-affinity` for [prefix caching](https://developers.cloudflare.com/workers-ai/features/prompt-caching/) discounts.
