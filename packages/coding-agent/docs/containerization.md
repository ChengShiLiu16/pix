...
```bash
cd /path/to/project
pi -e ~/.pix/agent/extensions/gondolin
```
...
Requirements: Node.js >= 23.6.0 for `@chengshiliu16/gondolin`, plus QEMU (requires installation through your package manager).
...
```bash
docker build -t pi-sandbox -f Dockerfile.pix .

docker run --rm -it \
  -e ANTHROPIC_API_KEY \
  -v "$PWD:/workspace" \
  -v pi-agent-home:/root/.pix/agent \
  pi-sandbox
```
