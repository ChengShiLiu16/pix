...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@earendil-works/pix-ai";
...
- `stopReason === "error"`
- `errorMessage` matches one of pix's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/earendil-works/pi-mono/blob/main/packages/ai/src/utils/overflow.ts))
