...
```typescript
import { AuthStorage, createAgentSession, ModelRegistry, SessionManager } from "@chengshiliu16/pix-coding-agent";
...
import { AuthStorage, ModelRegistry } from "@chengshiliu16/pix-coding-agent";
...
**Event Bus:** Extensions can communicate via `pix.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:
