...
```typescript
import { createAgentSession, ModelRuntime, SessionManager } from "@chengshiliu16/pix-coding-agent";
...
```bash
npm install @chengshiliu16/pix-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@chengshiliu16/pix-coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `pix.sendMessage()`.
...
The `Agent` class (from `@chengshiliu16/pix-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
import { ModelRuntime } from "@chengshiliu16/pix-coding-agent";
...
```typescript
import { InMemoryCredentialStore } from "@chengshiliu16/pix-ai";
import { createAgentSession, ModelRuntime } from "@chengshiliu16/pix-coding-agent";
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@chengshiliu16/pix-coding-agent";
...
import { createAgentSession, defineTool } from "@chengshiliu16/pix-coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `pix.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `pix.registerTool()`.
...
```typescript
import type { InlineExtension } from "@chengshiliu16/pix-coding-agent";
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@chengshiliu16/pix-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@chengshiliu16/pix-coding-agent";
