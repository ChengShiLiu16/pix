...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".pix", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".pix", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".pix", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".pix"), { recursive: true });
			writeFileSync(join(cwd, ".pix", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".pix"), { recursive: true, force: true });
