...
import { type AutocompleteProvider, CombinedAutocompleteProvider } from "@chengshiliu16/pix-tui";
...
describe("InteractiveMode.showLoadedResources", () => {
	...
	function createExtensionFixtures(): ExtensionFixture[] {
		return [
			{
				path: "/tmp/project/.pix/extensions/answer.ts",
				sourceInfo: createSourceInfo("/tmp/project/.pix/extensions/answer.ts", {
					source: "local",
					scope: "project",
					origin: "top-level",
					baseDir: "/tmp/project/.pix/extensions",
				}),
			},
			{
				path: "/tmp/project/.pix/extensions/local-index/index.ts",
				sourceInfo: createSourceInfo("/tmp/project/.pix/extensions/local-index/index.ts", {
					source: "local",
					scope: "project",
					origin: "top-level",
					baseDir: "/tmp/project/.pix/extensions",
			...
			{
				path: "/tmp/project/.pix/npm/node_modules/pi-markdown-preview/extensions/index.ts",
				sourceInfo: createSourceInfo("/tmp/project/.pix/npm/node_modules/pi-markdown-preview/extensions/index.ts", {
					source: "npm:pi-markdown-preview",
					scope: "project",
					origin: "package",
					baseDir: "/tmp/project/.pix/npm/node_modules/pi-markdown-preview",
				}),
			},
			{
				path: "/tmp/project/.pix/npm/node_modules/@scope/pi-scoped/extensions/index.ts",
				sourceInfo: createSourceInfo("/tmp/project/.pix/npm/node_modules/@scope/pi-scoped/extensions/index.ts", {
					source: "npm:@scope/pi-scoped",
					scope: "project",
					origin: "package",
					baseDir: "/tmp/project/.pix/npm/node_modules/@scope/pi-scoped",
				}),
			},
			{
				path: "/tmp/project/.pix/git/github.com/HazAT/pi-interactive-subagents/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.pix/git/github.com/HazAT/pi-interactive-subagents/extensions/index.ts",
					{
						source: "git:github.com/HazAT/pi-interactive-subagents",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.pix/git/github.com/HazAT/pi-interactive-subagents",
					},
				),
			},
			{
				path: "/tmp/project/.pix/git/github.com/HazAT/pi-interactive-subagents/extensions/subagents/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.pix/git/github.com/HazAT/pi-interactive-subagents/extensions/subagents/index.ts",
					{
						source: "git:github.com/HazAT/pi-interactive-subagents",
						scope: "project",
						origin: "package",
...
"[Extensions]
  project
    /tmp/project/.pix/extensions/answer.ts
    /tmp/project/.pix/extensions/local-index
  ...
  path
	...
	test("shows context paths relative to cwd while preserving full external paths", () => {
		const home = homedir();
		const cwd = path.join(home, "Development", "pix");
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			cwd,
			contextFiles: [{ path: path.join(home, ".pix", "agent", "AGENTS.md") }, { path: path.join(cwd, "AGENTS.md") }],
		});
		...
		expect(output).toContain("~/.pix/agent/AGENTS.md, AGENTS.md");
	...
	test("shows full context paths when expanded", () => {
		...
		const output = renderAll(fakeThis.chatContainer).replace(/\\/g, "/");
		expect(output).toContain("[Context]");
		expect(output).toContain("~/.pix/agent/AGENTS.md");
		expect(output).toContain("~/Development/pix/AGENTS.md");
		expect(output).not.toContain("~/.pix/agent/AGENTS.md, AGENTS.md");
	});
