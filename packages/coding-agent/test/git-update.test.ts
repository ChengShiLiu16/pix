...
describe("DefaultPackageManager git update", () => {
	...
	describe("temporary git sources", () => {
		it("should refresh cached temporary git sources when resolving", async () => {
			...
			const cachedDir = join(tmpdir(), "pix-extensions", `git-${gitHost}`, hash, gitPath);
			const extensionFile = join(cachedDir, "pix-extensions", "session-breakdown.ts");

			rmSync(cachedDir, { recursive: true, force: true });
			mkdirSync(join(cachedDir, "pix-extensions"), { recursive: true });
			writeFileSync(
				join(cachedDir, "package.json"),
				JSON.stringify({ pix: { extensions: ["./pix-extensions"] } }, null, 2),
			);
			...
			expect(getFileContent(cachedDir, "pix-extensions/session-breakdown.ts")).toBe("// fresh");
		});
		...
		it("should not refresh pinned temporary git sources", async () => {
			...
			expect(getFileContent(cachedDir, "pix-extensions/session-breakdown.ts")).toBe("// pinned");
		});
	});
	...
	describe("scope-aware update", () => {
		it("should not install locally when source is only registered globally", async () => {
			...
			const projectGitDir = join(tempDir, ".pix", "git", "github.com", "test", "extension");
