...
- pi-ai's old global API (`stream`/`complete`/`completeSimple`, `getModel`/`getModels`/`getProviders`, `registerApiProvider`, `getEnvApiKey`, ...) moved off the `@chengshiliu16/pix-ai` root entrypoint to `@chengshiliu16/pix-ai/compat`. Extensions are not affected at runtime: the extension loader resolves the pi-ai root to the compat entrypoint (a strict superset), so existing extensions keep working unchanged. Extension sources that typecheck against pi-ai's published types should switch those imports to `@chengshiliu16/pix-ai/compat` (or migrate to the new `createModels()`/provider-factory API). The compat entrypoint and the loader alias will be removed in a future release with a migration guide.
...
- Removed inherited selective-provider `@chengshiliu16/pix-ai/base` and `@chengshiliu16/pix-agent-core/base` entrypoints; use the root packages with explicit `Models` provider factories instead.
...
- Added inherited `@chengshiliu16/pix-ai/base` and `@chengshiliu16/pix-agent-core/base` entry points for selective provider registration in bundled applications ([#5348](https://github.com/earendil-works/pi/pull/5348) by [@FredKSchott](https://github.com/FredKSchott)).
...
- Added Claude Fable 5 model support inherited from `@chengshiliu16/pix-ai` for the Anthropic and Amazon Bedrock providers, with adaptive thinking and `xhigh` effort support.
...
- Added MiniMax-M3 model support inherited from `@chengshiliu16/pix-ai` for the `minimax` and `minimax-cn` direct providers ([#5313](https://github.com/earendil-works/pi/issues/5313)).
...
- Added custom Amazon Bedrock request header support inherited from `@chengshiliu16/pix-ai` ([#5178](https://github.com/earendil-works/pi-mono/pull/5178) by [@stephanmck](https://github.com/stephanmck)).
...
- Fixed Amazon Bedrock provider loading under strict package managers by inheriting the declared `@smithy/node-http-handler` dependency from `@chengshiliu16/pix-ai` ([#4842](https://github.com/earendil-works/pi/issues/4842)).
...
- Fixed OpenAI-compatible default output token requests inherited from `@chengshiliu16/pix-ai` to avoid reserving impossible context windows on servers such as vLLM ([#4675](https://github.com/earendil-works/pi/issues/4675)).
- Fixed OpenAI prompt cache keys inherited from `@chengshiliu16/pix-ai` to stay within the 64-character provider limit ([#4720](https://github.com/earendil-works/pi/issues/4720)).
...
- Fixed Xiaomi MiMo generated model metadata to replay assistant tool-call messages with `reasoning_content` for thinking-mode multi-turn requests, inherited from `@chengshiliu16/pix-ai` ([#4678](https://github.com/earendil-works/pi/issues/4678)).
...
- Fixed Anthropic-compatible API-key requests to ignore unrelated `ANTHROPIC_AUTH_TOKEN` environment values, avoiding invalid bearer credentials for providers such as Xiaomi MiMo inherited from `@chengshiliu16/pix-ai` ([#4342](https://github.com/earendil-works/pi/issues/4342)).
- Fixed Amazon Bedrock message conversion to skip unknown content blocks instead of failing the stream, inherited from `@chengshiliu16/pix-ai` ([#4223](https://github.com/earendil-works/pi/issues/4223)).
- Fixed Azure OpenAI Responses and OpenAI Responses error formatting to prefix HTTP status codes onto `errorMessage`, so transient 5xx and 429 errors are correctly matched by the agent-level auto-retry classifier inherited from `@chengshiliu16/pix-ai` ([#4232](https://github.com/earendil-works/pi/issues/4232)).
- Fixed OpenCode Go Kimi reasoning replay by normalizing streamed `reasoning` fields back to `reasoning_content` for OpenCode Go only, inherited from `@chengshiliu16/pix-ai` ([#4251](https://github.com/earendil-works/pi/issues/4251)).
- Fixed Xiaomi MiMo model metadata to use the OpenAI-compatible endpoints and `openai-completions` API, restoring multi-turn thinking/tool-call sessions inherited from `@chengshiliu16/pix-ai` ([#4505](https://github.com/earendil-works/pi/issues/4505)).
...
- Fixed OpenAI Codex generated model metadata to use the current upstream model list inherited from `@chengshiliu16/pix-ai` ([#4603](https://github.com/earendil-works/pi-mono/pull/4603) by [@mattiacerutti](https://github.com/mattiacerutti)).
- Fixed GitHub Copilot GPT model thinking metadata inherited from `@chengshiliu16/pix-ai` to map unsupported minimal thinking to low ([#4622](https://github.com/earendil-works/pi-mono/pull/4622) by [@mattiacerutti](https://github.com/mattiacerutti)).
...
- Fixed default output token requests for models whose advertised output limit is effectively their full context window, avoiding impossible provider requests inherited from `@chengshiliu16/pix-ai` ([#4614](https://github.com/earendil-works/pi/issues/4614)).
...
- Added image generation support from `@chengshiliu16/pix-ai`, including image generation APIs, image model metadata, and built-in OpenRouter image generation support ([#3887](https://github.com/earendil-works/pi-mono/pull/3887) by [@cristinaponcela](https://github.com/cristinaponcela)).
...
- **Startup timing instrumentation**: Set `PIX_TIMING=1` to see startup performance breakdown (interactive mode only).
...
- **Forking/Rebranding Support**: All branding (app name, config directory, environment variable names) is now configurable via `pixConfig` in `package.json`. Forks can change `pixConfig.name` and `pixConfig.configDir` to rebrand the CLI without code changes. Affects CLI banner, help text, config paths, and error messages. ([#95](https://github.com/badlogic/pi-mono/pull/95))
