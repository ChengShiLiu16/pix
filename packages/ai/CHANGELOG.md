...
- The root entrypoint (`@chengshiliu16/pix-ai`) is now core-only and side-effect free. The old global API moved to the temporary `@chengshiliu16/pix-ai/compat` entrypoint, a strict superset of the root: switching a file's import path is the only migration step. Moved symbols include `stream`/`complete`/`streamSimple`/`completeSimple`, `getModel`/`getModels`/`getProviders` (now deprecated aliases of `getBuiltinModel`/`getBuiltinModels`/`getBuiltinProviders` from `@chengshiliu16/pix-ai/providers/all`), `registerApiProvider`/`unregisterApiProviders`/`resetApiProviders`/`getApiProvider`, `getEnvApiKey`/`findEnvKeys`, `setBedrockProviderModule`, the per-API lazy stream wrappers (`anthropicMessagesApi`, ...), and the image-generation API.
...
- API implementation modules moved from `src/providers/` to `@chengshiliu16/pix-ai/api/*`, renamed by API id (`anthropic` -> `api/anthropic-messages`, `google` -> `api/google-generative-ai`, `mistral` -> `api/mistral-conversations`, `amazon-bedrock` -> `api/bedrock-converse-stream`), each exporting exactly `stream` and `streamSimple`. The old per-impl export names (`streamAnthropic`, `streamSimpleAnthropic`, ...) and legacy raw API subpaths (`./anthropic`, `./google`, `./openai-completions`, ...) are gone; import raw API implementations through `@chengshiliu16/pix-ai/api/*`.
- Removed the `@chengshiliu16/pix-ai/base` selective-provider entrypoint; use the root/core APIs with explicit `createModels()` collections and provider factories for isolated bundles.
...
- To keep the old global API temporarily, change imports from `@chengshiliu16/pix-ai` to `@chengshiliu16/pix-ai/compat`. The compat entrypoint preserves `stream`/`complete`, generated catalog reads, API registry helpers, env API-key helpers, lazy API wrappers, and image globals, but it will be removed in a future release.
...
- To call a raw API implementation directly, import from `@chengshiliu16/pix-ai/api/*` and pass a compatible model plus auth/options yourself. Raw API modules export `stream` and `streamSimple`; use `.result()` on the returned stream for `complete`/`completeSimple` behavior:

  ```ts
  import { streamSimple } from "@chengshiliu16/pix-ai/api/anthropic-messages";
  import { getBuiltinModel } from "@chengshiliu16/pix-ai/providers/all";
...
- One provider factory per built-in provider under `@chengshiliu16/pix-ai/providers/*` (e.g. `anthropicProvider()`, `openrouterProvider()`), plus `@chengshiliu16/pix-ai/providers/all` with `builtinProviders()`/`builtinModels()` and typed `getBuiltin*` catalog reads. Generated catalogs are split per provider, so importing one provider pulls one catalog; `sideEffects` metadata makes the package tree-shakeable.
...
- Added `@chengshiliu16/pix-ai/base` and direct provider registration exports for bundlers that want selective provider transports without root built-in registration ([#5348](https://github.com/earendil-works/pi/pull/5348) by [@FredKSchott](https://github.com/FredKSchott)).
