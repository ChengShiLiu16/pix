...
```bash
npm install @chengshiliu16/pix-ai
```
...
```typescript
import { Type, type Context, type Tool } from '@chengshiliu16/pix-ai';
import { builtinModels } from '@chengshiliu16/pix-ai/providers/all';
...
import { openaiProvider } from '@chengshiliu16/pix-ai/providers/openai';
import { openrouterProvider } from '@chengshiliu16/pix-ai/providers/openrouter';
import { amazonBedrockProvider } from '@chengshiliu16/pix-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@chengshiliu16/pix-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@chengshiliu16/pix-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@chengshiliu16/pix-ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@chengshiliu16/pix-ai';
...
```typescript
import { validateToolCall, type Tool } from '@chengshiliu16/pix-ai';
...
```typescript
import { builtinImagesModels } from '@chengshiliu16/pix-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@chengshiliu16/pix-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@chengshiliu16/pix-ai';
import { openAICompletionsApi } from '@chengshiliu16/pix-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@chengshiliu16/pix-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@chengshiliu16/pix-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@chengshiliu16/pix-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@chengshiliu16/pix-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@chengshiliu16/pix-ai';
import { anthropicProvider } from '@chengshiliu16/pix-ai/providers/anthropic';
import { openaiProvider } from '@chengshiliu16/pix-ai/providers/openai';
import { googleProvider } from '@chengshiliu16/pix-ai/providers/google';
...
- `@chengshiliu16/pix-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@chengshiliu16/pix-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@chengshiliu16/pix-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@chengshiliu16/pix-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@chengshiliu16/pix-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@chengshiliu16/pix-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@chengshiliu16/pix-ai/bedrock-provider';
...
```bash
npx @chengshiliu16/pix-ai login              # interactive provider selection
npx @chengshiliu16/pix-ai login anthropic    # login to specific provider
npx @chengshiliu16/pix-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@chengshiliu16/pix-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@chengshiliu16/pix-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@chengshiliu16/pix-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@chengshiliu16/pix-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@chengshiliu16/pix-ai`.
