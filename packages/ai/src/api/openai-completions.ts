...
type ResolvedOpenAICompletionsCompat = Omit<
	...
	"cacheControlFormat" | "deferredToolsMode" | "contentThinkingTags" | "toolCallContentPolicy"
> & {
	cacheControlFormat?: OpenAICompletionsCompat["cacheControlFormat"];
	deferredToolsMode?: OpenAICompletionsCompat["deferredToolsMode"];
	contentThinkingTags?: OpenAICompletionsCompat["contentThinkingTags"];
	toolCallContentPolicy?: OpenAICompletionsCompat["toolCallContentPolicy"];
};
...
function resolveCacheRetention(cacheRetention?: CacheRetention, env?: ProviderEnv): CacheRetention {
	...
	if (
		getProviderEnvValue("PIX_CACHE_RETENTION", env) === "long" ||
...
function pushContentThinkingTagDelta(
	...
	events.push(type === "text" ? { type: "text", delta } : { type: "thinking", delta });
}
	...
	(async () => {
		...
		try {
			...
			const emitTextDelta = (delta: string) => {
				...
				block.text += delta;
				stream.push({ type: "text_delta", contentIndex: getContentIndex(block), delta, partial: output });
			};
