...
function addCustomHeadersMiddleware(client: BedrockRuntimeClient, headers: Record<string, string>): void {
	...
	client.middlewareStack.add(middleware, { step: "build", name: "pix-ai-custom-headers", priority: "low" });
}
 ...
 * Defaults to "short" and uses PIX_CACHE_RETENTION for backward compatibility.
 */
function resolveCacheRetention(cacheRetention?: CacheRetention): CacheRetention {
	if (cacheRetention) {
		return cacheRetention;
	}
	if (typeof process !== "undefined" && process.env.PIX_CACHE_RETENTION === "long") {
		return "long";
	}
	return "short";
}
