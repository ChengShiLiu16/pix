# @chengshiliu16/pix-agent-core

Stateful agent with tool execution and event streaming. Built on `@chengshiliu16/pix-ai`.

## Installation

```bash
npm install @chengshiliu16/pix-agent-core
```

### SQLite session backends

The SQLite session backend and the `node:sqlite` adapter live in a separate package, `@chengshiliu16/pix-storage-sqlite-node`, so the core package does not pull in runtime builtins or native SQLite dependencies by default. The backend accepts a runtime-specific SQLite factory, allowing other storage backends to ship as their own packages in the future.
...
```typescript
declare module "@chengshiliu16/pix-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@chengshiliu16/pix-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@chengshiliu16/pix-agent-core";
