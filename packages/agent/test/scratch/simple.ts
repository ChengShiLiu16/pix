...
import { cloudflareAIGatewayProvider } from "@chengshiliu16/pix-ai/providers/cloudflare-ai-gateway";
...
const { promptTemplates: sourcedPromptTemplates } = await loadSourcedPromptTemplates<Source, SourcedPromptTemplate>(
	env,
	[source("project", join(env.cwd, ".pix/prompts")), source("user", join(homedir(), ".pix/agent/prompts"))],
