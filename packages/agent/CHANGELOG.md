...
- Moved the `uuidv7` export to `@chengshiliu16/pix-ai` ([#6834](https://github.com/earendil-works/pi/pull/6834) by [@xl0](https://github.com/xl0)).
- Replaced the optional `Agent` `streamFn` fallback with a required `streamFunction` and made low-level loop stream functions required, preventing `@chengshiliu16/pix-ai/compat` and all built-in providers from entering selective-provider bundles ([#6851](https://github.com/earendil-works/pi/issues/6851)).
...
- Removed the `@chengshiliu16/pix-agent-core/base` selective-provider entrypoint; use the root package with an explicit `Models` instance instead.
...
- Added `@chengshiliu16/pix-agent-core/base` for bundlers that want to pair the agent core with selective `@chengshiliu16/pix-ai/base` provider registration ([#5348](https://github.com/earendil-works/pi/pull/5348) by [@FredKSchott](https://github.com/FredKSchott)).
