...
```typescript
import { TUI, Text, Editor, ProcessTerminal, matchesKey } from "@chengshiliu16/pix-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with the `TUI` constructor option, `setShowHardwareCursor(true)`, or `PIX_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { CombinedAutocompleteProvider } from "@chengshiliu16/pix-tui";
...
```typescript
import { visibleWidth, truncateToWidth, wrapTextWithAnsi } from "@chengshiliu16/pix-tui";
...
```typescript
import { matchesKey, Key, truncateToWidth } from "@chengshiliu16/pix-tui";
...
```bash
PIX_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx test/chat-simple.ts
```
