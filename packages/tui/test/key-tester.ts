...
class KeyLogger implements Component {
	...
	private protocolName(): string {
		const terminal = this.terminal as ProcessTerminal & { modifyOtherKeysActive?: boolean };
		if (terminal.kittyProtocolActive) return "kitty";
		if (terminal.modifyOtherKeysActive) return "modifyOtherKeys";
		return "legacy";
	}
